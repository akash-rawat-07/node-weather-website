...
const forecast = (latitude, longitude, callback) => {
    ...
    request({url, json: true}, (error, {body}={}) => {  // using shorthand syntax {url} and destructuring {body}
        if(error){
            ...
            callback(undefined, `${body.current.weather_descriptions[0]}. It is currenty ${body.current.temperature} degress out. It feels like ${body.current.feelslike } degress out. And Humidity is ${body.current.humidity}%`)
        }
    })
}
